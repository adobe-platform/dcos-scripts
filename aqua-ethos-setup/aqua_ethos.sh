...
if [[ -z $TOKEN ]]; then
	echo "Unable to log in using user/password"
	exit 1
fi
...
while [[ "$EXISTING_PROFILE" == "200" && "$EXISTING_RULE" == "200" ]]; do
	echo "Profile and rule are still active..."
	TOKEN_RESP=$(curl --silent "$WEB_URL/v1/login" -H 'Content-Type: application/json' --data-binary '{"id":"administrator","password":"'$PASSWORD'"}')
	TOKEN=$(echo $TOKEN_RESP | jq -r .token)
...
echo "Profile ($EXISTING_PROFILE) or rule ($EXISTING_RULE) could not be found in Aqua, restarting to ensure compliance..."
exit 1
