...
function replaceConfigs {
	...
	if [[ ! -z "$ARTIFACTORY_URL_MC" ]]; then
		sed -i.bak "s@ETH_ARTIFACTORY_URL_MC@${ARTIFACTORY_URL_MC}@g" "$CONFIG_FILE"
		sed -i.bak "s@ETH_ARTIFACTORY_PREFIX_MC@${ARTIFACTORY_PREFIX_MC}@g" "$CONFIG_FILE"
		sed -i.bak "s@ETH_ARTIFACTORY_USERNAME_MC@${ARTIFACTORY_USERNAME_MC}@g" "$CONFIG_FILE"
		sed -i.bak "s@ETH_ARTIFACTORY_PASSWORD_MC@${ARTIFACTORY_PASSWORD_MC}@g" "$CONFIG_FILE"
	else
